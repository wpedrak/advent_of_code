...
while to_visit:
    ...
    for scaner2 in set(range(len(scaner_readouts))) - visited:
        ...
        iteration += 1
        print(f'{iteration:{padding}d}/{iterations_to_do:{padding}5d}')
        if overlap_count < 12:
            continue
...
all_vectors = functools.reduce(lambda x,y: x+y, scaner_readouts)
print(np.unique(np.array(all_vectors), axis=0).shape[0])
