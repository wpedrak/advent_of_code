...
class Monkey:
    def __init__(self, items: list[int], operation, divisor: int, target_true: int, target_false: int):
        ...
        self.inspections = 0
        self.worry_level_limiter = 1

    def inspect(self) -> tuple[int, int]:
        self.inspections += 1
        item = self.items.popleft()
        new_item = self.operation(item) % self.worry_level_limiter
    ...
    def set_worry_level_limiter(self, worry_level_limiter) -> None:
        self.worry_level_limiter = worry_level_limiter
...
lcm = functools.reduce(lambda x, y: x*y, (m.divisor for m in monkeys))
for monkey in monkeys:
    monkey.set_worry_level_limiter(lcm)
