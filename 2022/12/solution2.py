...
def run() -> None:
    ...
    print(shortest_path_length(grid, (ex, ey), 'a'))
...
def shortest_path_length(grid: list[list[str]], start: tuple[int, int], end_value: str) -> int:
    visited = {start}
    length = 0
...
def find_neighbours(grid: list[list[str]], point: tuple[int, int]) -> list[tuple[int, int]]:
    ...
    return [
            ...
            if ord(grid[y][x]) <= ord(grid[y+dy][x+dx]) + 1
            and grid[y+dy][x+dx] != '|'
        ]
