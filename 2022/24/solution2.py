...
def run() -> None:
    ...
    steps, blizzard = walk(walls, blizzard, (1, 0), (width-2, height-1))
    print(steps)
    steps2, blizzard = walk(walls, blizzard, (width-2, height-1), (1, 0))
    print(steps + steps2)
    steps3, blizzard = walk(walls, blizzard, (1, 0), (width-2, height-1))
...
def walk(walls: set[pnt], blizzard: list[tuple[pnt, str]], start: pnt, end: pnt) -> tuple[int, list[tuple[pnt, str]]]:
    ...
    while True:
        ...
        while to_visit:
            ...
            item_neighbours = neighbours(item, obstacles, right_wall_x, bot_wall_y)
            if end in item_neighbours:
...
def neighbours(point: pnt, obstacles: set[pnt], right_wall_x: int, bot_wall_y: int) -> list[pnt]:
    if point == (1, 0):
        return [p for p in [(1, 0), (1, 1)] if p not in obstacles]

    if point == (right_wall_x - 1, bot_wall_y):
        return [p for p in [(right_wall_x - 1, bot_wall_y), (right_wall_x - 1, bot_wall_y - 1)] if p not in obstacles]
