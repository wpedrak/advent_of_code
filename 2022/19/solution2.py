...
class Factory:
    ...
    def posible_targets(self, robots: rsc) -> list[int]:
        if robots[2]:
            return [1, 2, 3] # should be [0, 1, 2, 3]
        if robots[1]:
            return [1, 2] # should be [0, 1, 2]
        return [0, 1]

def run() -> None:
    result = 1
    for line in read_lines(file_name='2022/19/input.txt')[:3]:
...
def test(factory: Factory) -> int:
    to_visit = deque([((1, 0, 0, 0), None, (0, 0, 0, 0), target_robot, 32) for target_robot in [0, 1]])
    ...
    while to_visit:
        ...
        it += 1
        if not it % 100000:
            print(sum(robots), it//100000, len(to_visit))
