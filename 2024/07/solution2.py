...
def can_be_true(result: int, parts: list[int]) -> bool:
    def aux(acc: int, parts_idx: int) -> bool:
        if parts_idx == len(parts):
            return acc == result
        
        part = parts[parts_idx]
        return aux(acc + part, parts_idx+1) or aux(acc*part, parts_idx+1) or aux(int(str(acc) + str(part)), parts_idx+1)
